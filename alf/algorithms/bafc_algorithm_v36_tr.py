...
@alf.configurable
class BafcAlgorithmV36(OffPolicyAlgorithm):
    ...
    V36 implements model-free posterior sampling style exploration scheme over V2.
    ...
    def _parallel_fc_local_jacobian_all_groups(self, layer_cache):
        """Return the full local Jacobian for one cached ParallelFC layer.

        The returned tensor has shape ``[B, G, O, I]`` and represents the
        Jacobian of the layer output with respect to the layer input for every
        sample and every group. This mirrors the matrix-form propagation used by
        ``ReluMLP._compute_jac()`` and lets us keep all critic penultimate
        coordinates in matrix form at once.
        """
        weight = layer_cache['weight']
        batch_size = layer_cache['input'].shape[0]
        group_count, out_dim, _ = weight.shape
        row_scale = weight.new_ones((batch_size, group_count, out_dim))

        if layer_cache['use_ln']:
            gamma = layer_cache['gamma'].unsqueeze(0)
            rstd = layer_cache['rstd']
            row_scale = row_scale * gamma * rstd
            xhat = layer_cache['xhat']
            sum_weight = weight.sum(dim=-2, keepdim=True)
            xhat_weight = torch.einsum('bgo,goi->bgi', xhat, weight)
            centered_weight = (
                weight.unsqueeze(0)
                - sum_weight.unsqueeze(0) / float(out_dim)
                - torch.einsum('bgo,bgi->bgoi', xhat, xhat_weight) /
                float(out_dim))
        else:
            centered_weight = weight.unsqueeze(0)

        if layer_cache['is_relu']:
            row_scale = row_scale * layer_cache['mask']

        return row_scale.unsqueeze(-1) * centered_weight
    ...
    def _manual_critic_all_group_rows(self,
                                      ...
                                      joint_layers=None):
        """Manual critic Jacobians for all penultimate coords at once.

        This keeps the critic-side computation entirely in matrix form, like
        ``ReluMLP._compute_jac()``. For each sample ``n`` and group ``g`` it
        computes the full penultimate Jacobians ``d phi[n, g] / d a[n, g]`` and
        ``d phi[n, g] / d u[g]`` without a Python loop over feature
        coordinates.
        """
        del feature_coords  # full-feature fast path ignores coordinate subsets
        ...
        if len(obs_action_caches) == 0:
            total_in = obs_dim + action_detached.shape[-1]
            eye = torch.eye(
                total_in, device=obs.device, dtype=obs.dtype).view(
                    1, 1, total_in, total_in)
            j_obs_action = eye.expand(batch_size, group_count, -1, -1)
            j_obs_action = j_obs_action[..., obs_dim:]
        else:
            j_obs_action = None
            for layer_idx, cache in enumerate(obs_action_caches):
                local_jac = self._parallel_fc_local_jacobian_all_groups(cache)
                if layer_idx == 0:
                    j_obs_action = local_jac[..., obs_dim:]
                else:
                    j_obs_action = torch.einsum('bgop,bgpa->bgoa', local_jac,
                                                j_obs_action)

        x0 = torch.cat([actor_u, obs_action_encoding], dim=-1)
        x = x0
        joint_hidden_caches = []
        ...
        q, head_cache = self._forward_parallel_fc_all_groups(joint_layers[-1],
                                                             phi)
        q = q.squeeze(-1)

        actor_encoding_dim = actor_encoding_base.shape[-1]
        obs_action_dim = obs_action_encoding.shape[-1]
        if len(joint_hidden_caches) == 0:
            total_in = actor_encoding_dim + obs_action_dim
            eye = torch.eye(
                total_in, device=phi.device, dtype=phi.dtype).view(
                    1, 1, total_in, total_in)
            j_phi_input = eye.expand(batch_size, group_count, -1, -1)
        else:
            j_phi_input = None
            for layer_idx, cache in enumerate(joint_hidden_caches):
                local_jac = self._parallel_fc_local_jacobian_all_groups(cache)
                if layer_idx == 0:
                    j_phi_input = local_jac
                else:
                    j_phi_input = torch.einsum('bgop,bgpi->bgoi', local_jac,
                                               j_phi_input)

        j_phi_u = j_phi_input[..., :actor_encoding_dim]
        j_phi_obs_action = j_phi_input[..., actor_encoding_dim:]
        j_phi_action = torch.einsum('bgfo,bgoa->bgfa', j_phi_obs_action,
                                    j_obs_action)

        head_jac = self._parallel_fc_local_jacobian_all_groups(head_cache)
        dqda = torch.einsum('bgof,bgfa->bgoa', head_jac,
                            j_phi_action).squeeze(-2)
        dqu = torch.einsum('bgof,bgfu->bgu', head_jac, j_phi_u).sum(dim=0)

        feature_action_rows = None
        feature_du_rows = None
        if include_feature_rows:
            inv_batch = 1.0 / float(max(batch_size, 1))
            feature_action_rows = j_phi_action.permute(2, 0, 1, 3) * inv_batch
            feature_du_rows = j_phi_u.mean(dim=0).permute(1, 0, 2)
        ...
        result = dict(
            ...
            dqda=dqda,
            dqu=dqu,
            feature_action_rows=feature_action_rows,
            feature_du_rows=feature_du_rows)
        return result
    ...
    def _compute_manual_actor_step_rows(self,
        ...
        # One full matrix-form critic Jacobian pass for all groups and all
        # penultimate coordinates.
        rows = self._manual_critic_all_group_rows(
            observation,
            actor_encoding_base,
            action,
            feature_coords=None,
            include_feature_rows=compute_preupdate_grad_metric,
            obs_action_layers=obs_action_layers,
            joint_layers=joint_layers)

        dqda = rows['dqda']
        phi_t = rows['phi'] if compute_preupdate_grad_metric else None
        feature_dim = rows['phi'].shape[-1]
        grad_sq_phi = None
        if compute_preupdate_grad_metric:
            grad_sq_phi = action.new_zeros((self._num_actor_critic, feature_dim))

        up_u_rows_all = rows['dqu'].unsqueeze(0)
        if compute_preupdate_grad_metric:
            up_u_rows_all = torch.cat(
                [up_u_rows_all, rows['feature_du_rows']], dim=0)

        dqde_hidden = eval_hidden.new_zeros(eval_hidden.shape)
        dqde_action = eval_action_tensor.new_zeros(eval_action_tensor.shape)

        # The actor encoder remains a Transformer in the TR config. We therefore
        # keep the grouped batched VJP through the encoder, but supply all
        # penultimate coordinates at once per group.
        for group_idx in range(self._num_actor_critic):
            up_u_rows = up_u_rows_all[:, group_idx, :]
            # Keep the actor-encoder graph alive for the outer backward pass.
        ...
        if compute_preupdate_grad_metric and phi_t is not None:
            ...
            weighted_grad_sq = torch.clamp(a_inv_diag * grad_sq_phi, min=0.)
            c2 = torch.mean(
                torch.sqrt(
                    torch.clamp(weighted_grad_sq.sum(-1), min=0.) + 1e-12))
    ...
    def _compute_grad_generalization_trust_components_fast(self, observation):
        ...
        phi_mean = phi_t.mean(dim=0)
        a_inv_diag = torch.clamp(
            torch.diagonal(a_inv, dim1=-2, dim2=-1), min=0.)
        grad_sq_phi = self._batched_cutpoint_output_grad_sq_norm_fast(
            phi_mean, [cur_action, eval_hidden, eval_action], actor_cache,
            retain_graph=False)
    ...
    def _compute_preupdate_grad_generalization_trust_from_actor_step(
        ...
        Unlike the earlier sampled-row prototype, this path uses the full batch
        mean penultimate feature vector ``phi_t.mean(dim=0)`` and computes all
        feature-coordinate gradient norms at once.
        """
        if phi_t is None:
            return None
        with torch.no_grad():
    ...
    def _compute_grad_generalization_trust_components(self, observation):
        if self._supports_fast_grad_metric_last_two():
            return self._compute_grad_generalization_trust_components_fast(
                observation)
        raise RuntimeError("BafcAlgorithmV36 fell back to legacy autograd grad-metric computation.")
    ...
    def _actor_train_step(self,
        ...
        if use_manual_rows and self._actor_eval_type == 'last_two':
            try:
                ...
                self._pending_preupdate_grad_trust = None
                raise RuntimeError(
                    "BafcAlgorithmV36 manual actor-step row-VJP failed; "
                    "autograd fallback in actor_train_step is disallowed."
                ) from err
        ...
        if should_compute_trust_metrics:
            ...
            if isinstance(observation, torch.Tensor):
                ...
                if self._pending_preupdate_grad_trust is not None:
                    self._last_grad_trust = self._pending_preupdate_grad_trust.detach()
                elif self._enable_grad_actor_extend_gate:
                    self._last_grad_trust = (
                        self._compute_grad_generalization_trust_metric(
                            observation).detach())
                else:
