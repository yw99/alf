...
@alf.configurable
class BafcAlgorithmV3TR2(OffPolicyAlgorithm):
    ...
    def __init__(self,
        ...
        if trust_metric_target_obs_cache_size is None:
            trust_metric_target_obs_cache_size = 4 * trust_metric_num_obs
        assert trust_metric_target_obs_cache_size >= 1, (
            "trust_metric_target_obs_cache_size must be >= 1")
    ...
    def _flatten_metric_observations(self, observation):
    ...
    def _flatten_metric_actions(self, action):
    ...
    def _sample_metric_observation_action(self, observation, action):
        obs = self._flatten_metric_observations(observation)
        act = self._flatten_metric_actions(action)
        if not isinstance(obs, torch.Tensor) or not isinstance(act, torch.Tensor):
            return (), ()
        num_samples = min(obs.shape[0], act.shape[0])
        if num_samples == 0:
            return (), ()
        obs = obs[:num_samples]
        act = act[:num_samples]
        if num_samples > self._trust_metric_num_obs:
            idx = torch.randperm(num_samples, device=obs.device)[
                :self._trust_metric_num_obs]
            obs = obs[idx]
            act = act[idx.to(act.device)]
        return obs, act
    ...
    def _append_target_metric_observations(self, observation):
        ...
        if not isinstance(obs, torch.Tensor):
            return
        obs = obs.detach()
        cache = self._target_metric_observation_cache
        if isinstance(cache, torch.Tensor):
            if cache.device != obs.device:
                cache = cache.to(obs.device)
            obs = torch.cat([cache, obs], dim=0)
        if obs.shape[0] > self._trust_metric_target_obs_cache_size:
            obs = obs[-self._trust_metric_target_obs_cache_size:]
        self._target_metric_observation_cache = obs

    def _sample_target_metric_observations(self, fallback_observation):
        cache = self._target_metric_observation_cache
        obs = cache if isinstance(cache, torch.Tensor) else fallback_observation
        return self._sample_metric_observations(obs)
    ...
    def rollout_step(self, inputs: TimeStep, state: BafcState):
        ...
        self._append_target_metric_observations(inputs.observation)
