#!/bin/bash
# Launch humanoid:walk TD3 jobs on seeds 0, 1, 2, and 3. Every job uses
...
# run_humanoid_run_rlpd_seed0123-4g.sh. TD3 is configured with the same
...
#
# Usage: bash run_humanoid_walk_td3_seed0123-4g.sh [options]
...
#
# Example:
#   bash run_humanoid_walk_td3_seed0123-4g.sh --dry-run
...
ROOT_DIR="${BASE_DIR}/humanoid_walk/td3_seed0123_4g/num_critics${NUM_CRITICS}_sampled_target${NUM_SAMPLED_CRITIC_TARGETS}_actor_${ACTOR_CRITIC_AGGREGATION}_critic_ln${CRITIC_USE_LN}_critic_utd${CRITIC_UTD}"

cat <<EOF
Starting humanoid:walk TD3 seeds 0, 1, 2, and 3
...
if [[ "${DRY_RUN}" == "True" ]]; then
    ...
    echo "Launched four humanoid:walk TD3 4-GPU jobs: ${PIDS[*]}"
