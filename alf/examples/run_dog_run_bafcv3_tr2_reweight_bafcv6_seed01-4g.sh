#!/bin/bash
# Launch dog:run BAFCv3_TR2 with critic reweighting and BAFCv6 on seeds 0,1.
# Each job uses all four configured GPUs through DDP. The four jobs run in
# parallel on unique torch.distributed master ports.
#
# Usage: bash run_dog_run_bafcv3_tr2_reweight_bafcv6_seed01-4g.sh [options]
...
#       --seeds CSV          Comma-separated random seeds (default: 0,1)
#       --dry-run            Print commands without launching jobs
#   -h, --help               Show this help message
#
# Example:
#   bash run_dog_run_bafcv3_tr2_reweight_bafcv6_seed01-4g.sh --dry-run
...
for seed in "${SEEDS[@]}"; do
    if [[ ! "${seed}" =~ ^[0-9]+$ ]]; then
        echo "--seeds must be nonnegative integers, got: ${SEEDS[*]}" >&2
        exit 1
    fi
done
NUM_JOBS=$((2 * ${#SEEDS[@]}))
...
ENV_DIR="${ENV_NAME//:/_}"
ROOT_DIR="${BASE_DIR}/${ENV_DIR}/bafcv3_tr2_reweight_bafcv6_seed01_4g"

cat <<EOF
Starting dog:run TR2-reweight/BAFCv6 comparison
  ...
  GPUs per job: ${GPUS}
  Ports: ${BASE_PORT}-$((BASE_PORT + NUM_JOBS - 1))
  Dry run: ${DRY_RUN}
EOF
echo ""
...
for seed in "${SEEDS[@]}"; do
    ...
    TR2_COMMAND=(
        "${PYTHON_BIN}" -m alf.bin.train --conf "${TR2_CONF_FILE}" --root_dir "${TR2_RUN_DIR}"
        "${COMMON_ARGS[@]}"
    ...
    V6_COMMAND=(
        "${PYTHON_BIN}" -m alf.bin.train --conf "${V6_CONF_FILE}" --root_dir "${V6_RUN_DIR}"
        "${COMMON_ARGS[@]}"
    ...
    launch_job "BAFCv3_TR2 reweight, seed ${seed}" "$((BASE_PORT + port_offset))" \
        "${TR2_RUN_DIR}" "${TR2_COMMAND[@]}"
    ((port_offset += 1))
    launch_job "BAFCv6 reweight, seed ${seed}" "$((BASE_PORT + port_offset))" \
