#!/bin/bash
# Launch humanoid:walk BAFCv3_TR2 critic-reweighting jobs on seeds 0, 1, 2,
# and 3. Every job uses all four configured GPUs through DDP. The four jobs
# run in parallel on unique torch.distributed master ports.
#
# These settings match the BAFCv3_TR2 condition in
# run_humanoid_walk_rlpd_bafcv3_seed23-4g.sh except that critic reweighting is
# enabled. Results use a separate experiment directory.
#
# Usage: bash run_humanoid_walk_bafcv3_tr2_reweight_seed0123-4g.sh [options]
...
#
# Example:
#   bash run_humanoid_walk_bafcv3_tr2_reweight_seed0123-4g.sh --dry-run
...
TR2_ENABLE_CRITIC_REWEIGHTING=True
...
if [[ ! -f "${BAFCV3_TR2_CONF}" ]]; then
    echo "Config file not found: ${BAFCV3_TR2_CONF}" >&2
    exit 1
fi
...
Starting humanoid:walk BAFCv3_TR2 critic-reweighting seeds 0, 1, 2, and 3
  ...
  Eval trust max: ${TR2_EVAL_TRUST_MAX}
  Eval trust max decay: ${TR2_EVAL_TRUST_MAX_DECAY}
  Critic reweighting: ${TR2_ENABLE_CRITIC_REWEIGHTING}
  Critic reweighting beta: ${TR2_CRITIC_REWEIGHTING_BETA}
  Critic reweighting ridge: ${TR2_CRITIC_REWEIGHTING_RIDGE}
  Critic reweighting solver iterations: ${TR2_CRITIC_REWEIGHTING_SOLVER_ITERS}
  Critic reweighting max weight: ${TR2_CRITIC_REWEIGHTING_MAX_WEIGHT}
  Dry run: ${DRY_RUN}
EOF
echo ""
...
for seed in "${SEEDS[@]}"; do
    launch_job "${seed}" "$((BASE_PORT + port_offset))"
    ((port_offset += 1))
done
...
if [[ "${DRY_RUN}" == "True" ]]; then
    ...
    echo "Launched four BAFCv3_TR2 critic-reweighting 4-GPU jobs: ${PIDS[*]}"
