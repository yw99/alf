#!/bin/bash
# Launch dog:stand RLPD jobs on seeds 0, 1, 2, and 3. Every job uses all
# four configured GPUs through DDP. The four jobs run in parallel on unique
# torch.distributed master ports.
#
# Usage: bash run_dog_stand_rlpd_seed0123-4g.sh [options]
...
#
# Example:
#   bash run_dog_stand_rlpd_seed0123-4g.sh --dry-run
...
ENV_DIR="${ENV_NAME//:/_}"
ROOT_DIR="${BASE_DIR}/${ENV_DIR}/rlpd_seed0123_4g/critic_utd${CRITIC_UTD}"

cat <<EOF
Starting dog:stand RLPD seeds 0, 1, 2, and 3
...
if [[ "${DRY_RUN}" == "True" ]]; then
    ...
    echo "Launched four dog:stand RLPD 4-GPU jobs: ${PIDS[*]}"
