...
# Sources: dog:walk seeds 0/1; UTD 11 at 75k/105k, UTD 3 at 105k; skipping on.
set -euo pipefail
...
GPUS=0,1,2,3
DRY_RUN=false
JOBS=(0:75:11 0:105:3 0:105:11 1:75:11 1:105:3 1:105:11)
...
TR2 calibration, and end at absolute 150000 environment steps per rank.
...
build_command() {
    ...
    source="/workspace/server2_copy/dog_bafcv3_s${seed}/train/algorithm/ckpt-$((horizon * 1001))"
    ...
    COMMAND=("$PYTHON_BIN" -m alf.bin.train_bafcv3_tr2_restart
        ...
        --calibration-seed 20260917 --final-env-steps-per-rank 150000
        --worker-gpus "$GPUS")
}
...
runs = []
for path in sorted(root.glob('dog_walk_*/restart_manifest.json')):
...
echo "To monitor: tail -n 30 -f $ROOT_DIR/dog_walk_*/out.log"
