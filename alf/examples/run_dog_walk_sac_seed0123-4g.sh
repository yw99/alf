#!/bin/bash
# Launch SAC on dog:walk for seeds 0, 1, 2, and 3. Each seed uses all four
# configured GPUs through DDP, and all four jobs run in parallel using unique
# torch.distributed master ports.
#
# Usage: bash run_dog_walk_sac_seed0123-4g.sh [options]
...
#
# Example:
#   bash run_dog_walk_sac_seed0123-4g.sh --dry-run
...
cat <<EOF
Starting SAC on dog:walk for seeds 0, 1, 2, and 3
  Config: ${CONF_FILE}
...
if [[ "${DRY_RUN}" == "True" ]]; then
    ...
    echo "Launched four dog:walk SAC 4-GPU jobs: ${PIDS[*]}"
