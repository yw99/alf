#!/bin/bash
# Run TD3 tasks in order: humanoid:stand, dog:stand, dog:walk.
...
#
# Usage: bash run_td3_tasks_seed012-4g.sh [options]
#   -d, --dir BASE_DIR       Base results directory (default: /workspace/alf_results)
#   -n, --steps NUM_STEPS    Override steps per seed for all tasks (defaults: humanoid 600000, dog 800000)
...
#
# Results: BASE_DIR/<domain>/<task>/td3_dmc_4g/seed_<seed>/out.log
...
# Example:
#   bash alf/examples/run_td3_tasks_seed012-4g.sh --dry-run
#   bash alf/examples/run_td3_tasks_seed012-4g.sh --gpus 0,1,2 --worker-gpus 0,1,2,0
...
BASE_HTTP_PORT=8080
DRY_RUN=False
TASKS=(humanoid:stand dog:stand dog:walk)
TASK_STEPS=(600000 800000 800000)
SEEDS=(0 1 2)

# Match run_humanoid_run_td3_seed0123-4g.sh.
NUM_CRITICS=10
...
log "Steps per seed: ${NUM_ENV_STEPS:-humanoid=600000 dog=800000}; checkpoints: $NUM_CHECKPOINTS; results: $BASE_DIR"

# Check every destination before starting the queue; mkdir below also detects races.
for task in "${TASKS[@]}"; do
    for seed in "${SEEDS[@]}"; do
        run_dir="$BASE_DIR/${task/:/\/}/td3_dmc_4g/seed_$seed"
