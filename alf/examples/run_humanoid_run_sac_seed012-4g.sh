#!/bin/bash
# Launch SAC on humanoid:run for seeds 0, 1, and 2. Each seed uses four
...
#
# Usage: bash run_humanoid_run_sac_seed012-4g.sh [options]
...
#
# Example:
#   bash run_humanoid_run_sac_seed012-4g.sh --dry-run
...
cat <<EOF
Starting SAC on humanoid:run for seeds 0, 1, and 2
  Config: ${CONF_FILE}
...
if [[ "${DRY_RUN}" == "True" ]]; then
    ...
    echo "Launched three humanoid:run SAC jobs (${NUM_WORKERS} DDP workers each): ${PIDS[*]}"
