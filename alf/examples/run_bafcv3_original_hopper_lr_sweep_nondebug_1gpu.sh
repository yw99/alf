#!/bin/bash
# Launcher for original BAFCv3 hopper learning-rate sweep runs.
#
# Usage: bash run_bafcv3_original_hopper_lr_sweep_nondebug_1gpu.sh [options]
#   -e, --env ENV_NAME                 DMC environment (default: hopper:hop)
#   -d, --dir BASE_DIR                 Base results directory (default: /root/alf_results_v7_original_algo)
#   -n, --steps NUM_STEPS              Total env steps (default: 1000000)
#   -s, --seed SEED                    Seed for each run (default: 4)
...
#
# Example:
#   bash run_bafcv3_original_hopper_lr_sweep_nondebug_1gpu.sh --seed 0
...
echo "Launching ${#GPU_IDS[@]} original BAFCv3 hopper LR sweep run(s)"
