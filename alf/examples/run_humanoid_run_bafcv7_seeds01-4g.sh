#!/bin/bash
# Launch the two BAFCv7 presets for humanoid:run and seeds 0,1.
#
# Usage: bash run_humanoid_run_bafcv7_seeds01-4g.sh [options]
...
#       --base-port PORT     First of four DDP ports (default: 29600)
#       --policy-features M  mean_log_std or action_quantiles (default: mean_log_std)
#       --dry-run            Print all four commands without launching
#   -h, --help               Show this help
#
# Example:
#   bash run_humanoid_run_bafcv7_seeds01-4g.sh --dry-run
...
if [[ ! "${BASE_PORT}" =~ ^[1-9][0-9]*$ ]] || (( BASE_PORT + 3 > 65535 )); then
    echo "--base-port must leave room for four valid ports" >&2
    exit 1
fi
...
ROOT_DIR="${BASE_DIR}/humanoid_run/bafcv7_policy_features_4g"

echo "Starting BAFCv7 humanoid:run sweep"
...
echo ""
if [[ "${DRY_RUN}" == "True" ]]; then
    echo "Dry run complete; emitted four jobs and launched none."
else
    echo "Launched four BAFCv7 four-GPU jobs: ${PIDS[*]}"
    echo "Launcher is not waiting for completion."
fi
echo "Results: ${ROOT_DIR}/${POLICY_FEATURES}/{ensemble_base/lambda0.10,single_seeded/lambda0.90}/actor_utd${ACTOR_UTD}_critic_utd${CRITIC_UTD}/seed_{0,1}"
