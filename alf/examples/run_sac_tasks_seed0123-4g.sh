#!/bin/bash
# Run SAC tasks in order: dog:fetch, dog:run, dog:stand, humanoid:stand.
...
#
# Usage: bash run_sac_tasks_seed0123-4g.sh [options]
...
#
# Results: BASE_DIR/<domain>/<task>/sac_dmc_4g/seed_<seed>/out.log
...
# Example:
#   bash alf/examples/run_sac_tasks_seed0123-4g.sh --dry-run
#   bash alf/examples/run_sac_tasks_seed0123-4g.sh --gpus 0,1,2 --worker-gpus 0,1,2,0
...
BASE_HTTP_PORT=8080
DRY_RUN=False
TASKS=(dog:fetch dog:run dog:stand humanoid:stand)
SEEDS=(0 1 2)
...
[[ "$NUM_ENV_STEPS" =~ ^[1-9][0-9]*$ ]] || die "--steps must be a positive integer"
...
log "Steps per seed: $NUM_ENV_STEPS; checkpoints: $NUM_CHECKPOINTS; results: $BASE_DIR"

# Check every destination before starting the queue; mkdir below also detects races.
for task in "${TASKS[@]}"; do
    for seed in "${SEEDS[@]}"; do
        run_dir="$BASE_DIR/${task/:/\/}/sac_dmc_4g/seed_$seed"
