#!/bin/bash
# Launch humanoid:run TD3 jobs on seeds 0, 1, 2, and 3. Every job uses
...
#
# Usage: bash run_humanoid_run_td3_seed0123-4g.sh [options]
...
#
# Example:
#   bash run_humanoid_run_td3_seed0123-4g.sh --dry-run
...
ROOT_DIR="${BASE_DIR}/humanoid_run/td3_seed0123_4g/num_critics${NUM_CRITICS}_critic_utd${CRITIC_UTD}"

cat <<EOF
Starting humanoid:run TD3 seeds 0, 1, 2, and 3
...
if [[ "${DRY_RUN}" == "True" ]]; then
    ...
    echo "Launched four humanoid:run TD3 4-GPU jobs: ${PIDS[*]}"
