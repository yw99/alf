#!/bin/bash
# Launch the BAFCv6 random-target condition on humanoid:stand seeds 0–3.
...
#
# Usage: bash run_humanoid_stand_bafcv6_seed0123-4g.sh [options]
...
#
# Example:
#   bash run_humanoid_stand_bafcv6_seed0123-4g.sh --dry-run
...
ROOT_DIR="${BASE_DIR}/humanoid_stand/bafcv6_seed0123_4g/num_actor_critic${NUM_ACTOR_CRITIC}_num_sampled_critics_for_actor${NUM_SAMPLED_CRITICS_FOR_ACTOR}_num_sampled_critic_targets${NUM_SAMPLED_CRITIC_TARGETS}/critic_utd${CRITIC_UTD}"
...
cat <<EOF
Starting BAFCv6 humanoid:stand seeds 0–3
  Config: ${CONF_FILE}
  Root dir: ${ROOT_DIR}
