#!/bin/bash
# Launch the BAFCv6 random-target condition on humanoid:walk seeds 2 and 3.
# Each job uses all configured GPUs through DDP; both jobs run in parallel.
# The configuration matches the BAFCv6 condition in
# run_bafcv3_tr_v6_random_target_sweep_seed01-4g.sh.
#
# Usage: bash run_humanoid_walk_bafcv6_seed23-4g.sh [options]
...
#
# Example:
#   bash run_humanoid_walk_bafcv6_seed23-4g.sh --dry-run
...
ROOT_DIR="${BASE_DIR}/humanoid/bafcv3_tr2_v6_random_target_sweep_4g/num_actor_critic${NUM_ACTOR_CRITIC}_num_sampled_critics_for_actor${NUM_SAMPLED_CRITICS_FOR_ACTOR}_num_sampled_critic_targets${NUM_SAMPLED_CRITIC_TARGETS}/critic_utd${CRITIC_UTD}"

cat <<EOF
Starting BAFCv6 humanoid:walk seeds 2 and 3
  Config: ${CONF_FILE}
  Root dir: ${ROOT_DIR}
...
if [[ "${DRY_RUN}" == "True" ]]; then
    ...
    echo "Launched two BAFCv6 4-GPU jobs: ${PIDS[*]}"
