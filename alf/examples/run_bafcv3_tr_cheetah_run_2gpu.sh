...
# Runs either a 2-job threshold sweep or a single original-baseline job.
#
...
#       --original-algo                Disable trust metrics and both trust gates; launch one job on GPU A
#   -h, --help                         Show this help message
#
# Example:
#   bash run_bafcv3_tr_cheetah_run_2gpu.sh --gpus 0,1 --eval-a 2 --delta-a 4 --eval-b 5 --delta-b 10 --rollout-hold-cap 20 --critic-extend-cap 5
...
GPU_A=0
GPU_B=1
EVAL_TRUST_MAX_A=1e-4
DELTA_TRUST_MAX_A=1e-4
...
if [[ "${ORIGINAL_ALGO}" == "true" ]]; then
    ...
    RUN_A_DIR="${ROOT_BASE}/A_eval${EVAL_TRUST_MAX_A}_delta${DELTA_TRUST_MAX_A}_cap${ROLLOUT_HOLD_CAP}_gcap${CRITIC_EXTEND_CAP}_seed${SEED_A}"
    RUN_B_DIR="${ROOT_BASE}/B_eval${EVAL_TRUST_MAX_B}_delta${DELTA_TRUST_MAX_B}_cap${ROLLOUT_HOLD_CAP}_gcap${CRITIC_EXTEND_CAP}_seed${SEED_B}"
fi
if [[ "${ORIGINAL_ALGO}" == "true" ]]; then
    mkdir -p "${RUN_DIR}"
else
...
if [[ "${ORIGINAL_ALGO}" == "true" ]]; then
    CUDA_VISIBLE_DEVICES="${GPU_A}" python -m alf.bin.train \
        --conf "${CONF_FILE}" \
        --root_dir "${RUN_DIR}" \
    ...
    CUDA_VISIBLE_DEVICES="${GPU_B}" python -m alf.bin.train \
        --conf "${CONF_FILE}" \
        --root_dir "${RUN_B_DIR}" \
        --conf_param "debug_mode=True" \
