#!/bin/bash
# Launch the BAFCv6 random-target condition on dog:fetch seeds 0–3.
...
# Fresh runs use bafcv6_seed0123_4g_rerun_20260916; existing results are refused.
#
# Usage: bash run_dog_fetch_bafcv6_seed0123-4g.sh [options]
...
#
# Example:
#   bash run_dog_fetch_bafcv6_seed0123-4g.sh --dry-run
...
ROOT_DIR="${BASE_DIR}/dog_fetch/bafcv6_seed0123_4g_rerun_20260916/num_actor_critic${NUM_ACTOR_CRITIC}_num_sampled_critics_for_actor${NUM_SAMPLED_CRITICS_FOR_ACTOR}_num_sampled_critic_targets${NUM_SAMPLED_CRITIC_TARGETS}/critic_utd${CRITIC_UTD}"


# Reserve a new experiment directory before launching any seeds.
# A repeated invocation must use a different --dir to preserve existing logs.
if [[ -e "${ROOT_DIR}" || -L "${ROOT_DIR}" ]]; then
    echo "Results directory already exists: ${ROOT_DIR}" >&2
    echo "Use --dir with a new base directory to start fresh runs." >&2
    exit 1
fi
...
cat <<EOF
Starting BAFCv6 dog:fetch seeds 0–3
  Config: ${CONF_FILE}
  Root dir: ${ROOT_DIR}
