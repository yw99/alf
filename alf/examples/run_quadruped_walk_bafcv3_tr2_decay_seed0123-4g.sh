#!/bin/bash
# Launch the quadruped:walk BAFCv3_TR2 jobs on seeds 0, 1, 2, and 3.
...
# run_quadruped_walk_bafcv3_tr2_seed01-4g.sh, except eval-trust-max decay is
# enabled. Results use a separate condition directory.
#
# Usage: bash run_quadruped_walk_bafcv3_tr2_decay_seed0123-4g.sh [options]
...
#
# Example:
#   bash run_quadruped_walk_bafcv3_tr2_decay_seed0123-4g.sh --dry-run
...
CONDITION="bafcv3_tr2_no_cadence_trust_max_decay/fixed_pairingFalse_num_sampled_critic${BAFCV3_NUM_SAMPLED_CRITICS}/critic_utd${BAFCV3_CRITIC_UTD}"

cat <<EOF
Starting dog:walk BAFCv3_TR2 seeds 0, 1, 2, and 3
...
if [[ "${DRY_RUN}" == "True" ]]; then
    ...
    echo "Launched four BAFCv3_TR2 4-GPU jobs: ${PIDS[*]}"
    echo "Launcher is not waiting for completion."
fi
echo "To monitor BAFCv3_TR2: tail -f ${ROOT_DIR}/${CONDITION}/seed_*/out.log"
